...
def calc_seasonality(attr : pd.DataFrame) -> float:
    """
    Calculates seasonality (a metric for when during the year it's rainiest)
    from NLDI attributes. Helper function for load_attributes.
...
def calc_soil_stats(attr : pd.DataFrame, bias : float, weights : Tuple[float, float, float, float, float, float, float]) -> float:
    """
    Does multiple linear regression (using provided weights and bias) on NLDI attributes. Helper function for load_attributes.
...
def calc_prec_stats(df : pd.DataFrame, p_mean : float) -> Tuple[list, list, list, list]:
    """
    Calculates precipitation-related attributes high_prec_freq, high_prec_dur, low_prec_freq,
    and low_prec_dur from meteorological forcings. Helper function for load_attributes.
...
def load_attributes(gauge_id : str, df : pd.DataFrame) -> pd.DataFrame:
    """
    Loads the attributes of a basin based on forcings, NLDI data, and calculations.

    Parameters
    ----------
    gauge_id : str
        The USGS gauge ID of the desired basin.
    df : pd.DataFrame
        A Pandas DataFrame containing daily precipitation data.
    ...
    -------
    pd.DataFrame
        A Pandas DataFrame containing labeled and adjusted (to match CAMELS) attributes.
    """
    # Get the COM ID and setup data dict to eventually turn into DataFrame.
    com_id = int(NLDI_DB.getfeature_byid("nwissite", f"USGS-{gauge_id}")["comid"].iloc[0])
    data = {}
    # These are the characteristics from NLDI that we will turn into attributes.
    ...
    data["p_seasonality"] = [calc_seasonality(attr)] # seasonality is calculated through multiple regression of monthly prcp
    ...
    data["slope_mean"] = [2.76435 * attr["TOT_BASIN_SLOPE"].iloc[0] - 3.20257] # unsure why but CAMELS and NLDI have very different slopes
    data["area_gages2"] = list(attr["TOT_BASIN_AREA"])
    # Soil statistics also vary drastically between NLDI and CAMELS, convert most through multiple regression.
    data["geol_permeability"] = [np.log10(attr["TOT_PERMAVE"].iloc[0]) - 15] # logarithm and convert units
    ...
    # Some stats are missing from NLDI so just take the average of CAMELS instead.
...
def load_gridmet(geometry : Geometry, dates : Tuple[str, str]) -> pd.DataFrame:
    """
    Loads some GridMET data. Helper function for load_forcings.

    Parameters
    ----------
    geometry : Geometry
        The geometry of the area to load data for.
    dates : Tuple[str, str]
        The start and end dates to load data for, in "YYYY-MM-DD" format.
    
    Returns
    -------
    pd.DataFrame
        The specified meteorological data, sourced from GridMET.
    """
    var = ["pr", "pet", "srad", "tmmx", "tmmn", "sph"]
    xarr = gridmet.get_bygeom(geometry, dates, variables=var, snow=True)
    # Average data across spatial dimensions (keeping time) and convert to DataFrame.
...
def load_forcings(gauge_id : str) -> Tuple[pd.DataFrame, int]:
    """
    Loads all meteorological data for a basin.
    ...
    -------
    pd.DataFrame
        A Pandas DataFrame containing labeled (to match CAMELS) forcings.
    int
        The area of the basin, in square meters.
    """
    # Set up COM ID and geometry from NLDI data.
    geometry = NLDI_DB.get_basins(gauge_id).geometry.iloc[0]
    com_id = int(NLDI_DB.getfeature_byid("nwissite", f"USGS-{gauge_id}")["comid"].iloc[0])
    # Calculate area in square meters for use in load_streamflow.
    area = int(NLDI_DB.get_characteristics(["TOT_BASIN_AREA"], com_id)["TOT_BASIN_AREA"].iloc[0] * 1000000)
    if gauge_id == "01022500": # this one is weird for some reason, other uncaught ones may exist
        area = 587675987
    # Because there's so much data pre-spatial-average, load and reduce 5 years at a time.
    dfs = []
    for i in range(8):
...
def online_json_to_df(url : str) -> pd.DataFrame:
    """
    Opens a URL that should point to a USGS JSON file, and converts it to
    a DataFrame. Helper function for load_streamflow.

    Parameters
    ----------
    url : str
        The URL where the USGS JSON is located.
    
    Returns
    -------
    pd.DataFrame
        A Pandas DataFrame containing the data stored in the JSON.
    """
    with request.urlopen(url) as f:
        usgs = f.read().decode('utf-8') # unsure if the decode is necessary
        jdb = json.loads(usgs)
    # The JSON contains a lot of metadata, get past it for the actual streamflow.
...
def load_streamflow(gauge_id : str, area : int) -> pd.Series:
    """
    Loads all streamflow data for a basin.
    ...
    -------
    pd.Series
        A Pandas Series containing the streamflow, indexed by DateTime.
    """
    # Using urllib, we open an online file in JSON format to access USGS streamflow data. This may
    # become deprecated, the USGS file is also not guaranteed to work for all basins (but it does for CAMELS)
    ...
    df = online_json_to_df(usgs_url)
    # Format the data to resemble CAMELS.
...
# From basin_list.txt, get a list of gauge IDs.
...
for basin in basin_list:
    ...
    if os.path.exists(attributes_path) and os.path.exists(timeseries_path):
        ...
        continue
    # Get GridMET data and process it, alongside NLDI, for attributes.
    ...
    # Re-index because GridMET doesn't include December 31 on leap years (why???).
    forcings = forcings.reindex(pd.date_range('01-01-1980', '12-31-2019'))
    forcings.index.names = ["date"]
    # Drop unnecessary data and duplicate GridMET since nh-model wants three sources of forcings.
    ...
    # Save forcings to a NetCDF file.
    xarr = df.to_xarray()
